<template>
  <div id="main_navigation_background" :style="{background:bg_colors[current_index]}">
    <!--
            选择课程的区域 和 广告选择区域。
    -->
    <div id="main_navigation">
      <div id="course_categories">
        <ul id="course_categories_ul">
          <li class="course_categories_li" v-for="(nav,index) in index_categories" :key="index">
            ...
            <div
              class="sub_course_categories_div"
              :style="{ top: index > 3 ? ((-57 * (index-4))-40)+'px' : '0px' }"
            >
              <p class="sub_course_categories_p">{{ nav.name }}</p>
              <div class="sub_course_categories_span_div">
                <span
                  v-for="(sub_category, s_c_index) in nav.tags"
                  :key="s_c_index"
          ...
          <li class="course_categories_footer">
            <div class="course_categories_footer_div">
              <a href="#41" class="course_categories_footer_a">训练营</a>
      ...
      <v-loader :source="index_banner">
        <transition-group name="ad_transition_group" tag="ul" class="ad_transition_group_ul">
          ...
          >
            <a class="ads_a" tagget="_blank" :href="ad['html_url']">
